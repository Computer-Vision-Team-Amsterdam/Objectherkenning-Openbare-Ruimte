...
def run_metadata_healthcheck_step(sparkSession, catalog, schema, job_process_time):
    ...
    valid_frame_metadata, invalid_frame_metadata = metadataHealthCheker.process_frame_metadata(bronze_frame_metadata=bronze_frame_metadata_df)
    tableManager.write_to_table(valid_frame_metadata, table_name="silver_frame_metadata")
    tableManager.write_to_table(invalid_frame_metadata, table_name="silver_frame_metadata_quarantine")
    tableManager.update_status(table_name="bronze_frame_metadata", job_process_time=job_process_time)

    bronze_detection_metadata_df = metadataHealthCheker.load_bronze_metadata(table_name="bronze_detection_metadata")
    valid_detection_metadata, invalid_detection_metadata = metadataHealthCheker.process_detection_metadata(bronze_detection_metadata_df)
    tableManager.write_to_table(valid_detection_metadata, table_name="silver_detection_metadata")
    tableManager.write_to_table(invalid_detection_metadata, table_name="silver_detection_metadata_quarantine")
