...
def _add_metadata_and_permits(
    ...
    for src_name in frame_gdf["frame_src"].unique():
        ...
        meta_gdf = load_metadata_gdf(meta_path).set_index("new_frame_id")["geometry"]
...
def load_frame_gdf(
    ...
    return _add_metadata_and_permits(
        frame_gdf, metadata_folder, decos_gdf, decos_buffer
    )


def sample_frames(
    frame_gdf: gpd.GeoDataFrame, n_frames: int, sampling_weight: float
) -> gpd.GeoDataFrame:
    weights = np.array(
        [
            sampling_weight if permit else 1 - sampling_weight
            for permit in frame_gdf["permit"]
        ]
    )
    weights = weights / np.sum(weights)

    if n_frames >= np.count_nonzero(weights):
        draw = np.where(weights > 0)[0]
        print(
            f"Cannot draw {n_frames} samples out of {np.count_nonzero(weights)} nonzero weights, sampling all nonzero."
        )
    else:
        draw = np.random.choice(
            np.arange(len(frame_gdf)), n_frames, p=weights, replace=False
        )
    sample = np.zeros_like(frame_gdf["permit"], dtype=bool)
    sample[draw] = True
    frame_gdf["sample"] = sample

    return frame_gdf


def copy_sample(frame_gdf: gpd.GeoDataFrame, output_folder: str):
    pathlib.Path(output_folder).mkdir(exist_ok=True, parents=True)

    print(f"Copying sample to {output_folder}")

    for i, row in frame_gdf.iterrows():
        if row["sample"]:
            frame = row["path"]
            target_path = os.path.join(output_folder, frame.name)
            shutil.copyfile(frame.as_posix(), target_path)


def run_sampling_job(
    input_folder: str, metadata_folder: str, decos_folder: str, output_folder: str
):
    ds_settings = settings["data_sampling"]
    n_frames = ds_settings["n_frames"]
    sampling_weight = ds_settings["sampling_weight"]
    decos_buffer = ds_settings["decos_buffer"]

    decos_gdf = decos_helper.load_and_combine_decos(decos_folder)
    frame_gdf = load_frame_gdf(input_folder, metadata_folder, decos_gdf, decos_buffer)
    frame_gdf = sample_frames(frame_gdf, n_frames, sampling_weight)
    ...
    copy_sample(frame_gdf, output_folder)

    metadata_file = os.path.join(output_folder, "metadata.gpkg")
    frame_gdf = frame_gdf.assign(path=frame_gdf["path"].apply(lambda p: p.as_posix()))
    print(f"Storing metadata in {metadata_file}")
    frame_gdf.to_file(metadata_file, driver="GPKG")
