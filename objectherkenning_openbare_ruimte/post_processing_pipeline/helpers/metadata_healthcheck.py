...
class MetadataHealthChecker:
    def __init__(self, spark, catalog, schema, job_process_time):
        ...
        print(f"02: Loaded {self.bronze_frame_metadata.count()} 'Pending' rows from {self.catalog}.{self.schema}.bronze_frame_metadata.")
        ...
        print(f"02: Loaded {self.bronze_detection_metadata.count()} 'Pending' rows from {self.catalog}.{self.schema}.bronze_detection_metadata.")
    ...
    def process_and_save_frame_metadata(self):
        ...
        valid_metadata.write.mode('append').saveAsTable(f'{self.catalog}.{self.schema}.silver_frame_metadata')
        print(f"02: Appended {valid_metadata.count()} rows to {self.catalog}.{self.schema}.silver_frame_metadata.")

        invalid_metadata.write.mode('append').saveAsTable(f'{self.catalog}.{self.schema}.silver_frame_metadata_quarantine')
        print(f"02: Appended {invalid_metadata.count()} rows to {self.catalog}.{self.schema}.silver_frame_metadata_quarantine.")
    ...
    def process_and_save_detection_metadata(self):
        ...
        valid_metadata.write.mode('append').saveAsTable(f'{self.catalog}.{self.schema}.silver_detection_metadata')
        print(f"02: Appended {valid_metadata.count()} rows to silver_detection_metadata.")

        invalid_metadata.write.mode('append').saveAsTable(f'{self.catalog}.{self.schema}.silver_detection_metadata_quarantine')
    ...
    def update_bronze_status(self, table_name):
        ...
        print(f"02: Updated 'Pending' status to 'Processed' in {self.catalog}.{self.schema}.{table_name}.")
